...
class Solution {
 public:
  bool possibleBipartition(int N, vector<vector<int>>& dislikes) {
    ...
    groups[dislikes[0][0]] = 0;
    groups[dislikes[0][1]] = 1;
    return dfs(groups, dislikes, 1);
  }

  bool dfs(vector<int>& groups, const vector<vector<int>>& dislikes, size_t start) {
    bool ret = true;
    for (size_t i = start, m = dislikes.size(); i < m; ++i) {
      const int& a = dislikes[i][0], b = dislikes[i][1];
      if (groups[a] != -1 && groups[b] != -1) {
        if (groups[a] == groups[b]) { return false; }
        return dfs(groups, dislikes, i+1);
      } else if (groups[a] == -1 && groups[b] != -1) {
        if (groups[b] == 1) { groups[a] = 0; }
        else { groups[a] = 1; }
        bool ret = dfs(groups, dislikes, i+1);
        if (!ret) { groups[a] = -1; }
        return ret;
      } else if (groups[a] != -1 && groups[b] == -1) {
        if (groups[a] == 1) { groups[b] = 0; }
        else { groups[b] = 1; }
        bool ret = dfs(groups, dislikes, i+1);
        if (!ret) { groups[b] = -1; }
        return ret;
