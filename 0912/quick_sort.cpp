...
class Solution {
 ...
 private:
  void quickSort(vector<int>& nums, int left, int right) {
    ...
    int pivot = getPivot(nums, left);
    vector<int> vec =  partition(nums, pivot, left, right);
    quickSort(nums, left, vec[1]);
    quickSort(nums, vec[0], right);
  }

  int getPivot(const vector<int>& nums, int left) {
    if (left == nums.size()-1) {
      return nums[left];
    }
  ...
  vector<int> partition(vector<int>& nums, int pivot, int left, int right) {
    while (left <= right) {
